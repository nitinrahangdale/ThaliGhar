import { Pencil as Edit2, TrendingUp, TrendingDown, Star, AlertTriangle } from 'lucide-react';
...
function MobileCardRow({ r, isBest, isWorst }: any) {
  ...
  return (
    <div className={`rounded-xl border p-4 space-y-3 ${
      isBest ? 'bg-amber-500/5 border-amber-500/30' : isWorst ? 'bg-rose-500/5 border-rose-500/30' : 'bg-slate-800 border-slate-700'
    }`}>
      {/* Date Header */}
      <div className="flex items-center justify-between pb-3 border-b border-slate-700">
        <div className="flex items-center gap-2">
          {isBest && <Star className="w-3 h-3 text-amber-400 shrink-0" />}
          {isWorst && <AlertTriangle className="w-3 h-3 text-rose-400 shrink-0" />}
          <span className="text-white font-semibold">
            {new Date(r.date).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: '2-digit' })}
          </span>
        </div>
        <button
          onClick={() => { setEditDate(r.date); setPage('entry'); }}
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <div>
            ...
            <p className="text-teal-400 text-sm font-medium">{fmt(r.cash_income)} / {fmt(r.online_income)}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <span className={`flex items-center gap-1 font-semibold ${r.total_profit < 0 ? 'text-rose-400' : 'text-emerald-400'}`}>
            {r.total_profit < 0 ? <TrendingDown className="w-3 h-3" /> : <TrendingUp className="w-3 h-3" />}
            {fmt(r.total_profit)}
          </span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 pt-2">
        <div className="flex-1 h-2 bg-slate-700 rounded-full overflow-hidden">
          <div
            ...
            style={{ width: `${Math.min(Math.abs(r.profit_percentage), 100)}%` }}
          />
        </div>
        <span className={`text-xs font-medium ${r.profit_percentage < 0 ? 'text-rose-400' : 'text-emerald-400'}`}>
    ...
    <>
      ...
      <div className="hidden md:block bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
        ...
        <div className="px-5 py-4 border-b border-slate-800 flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-400 text-xs mt-0.5">{data.length} records this month</p>
          </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-800">
                {['Date', 'Income', 'Cash', 'Online', 'Expenses', 'Profit', 'Margin', ''].map((h) => (
                  <th
                    key={h}
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`flex items-center gap-1 font-semibold ${
                          r.total_profit < 0 ? 'text-rose-400' : 'text-emerald-400'
                        }`}
                      >
                        {r.total_profit < 0 ? (
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <div className="w-16 h-1.5 bg-slate-700 rounded-full overflow-hidden">
                          <div
                            className={`h-full rounded-full ${
                              r.profit_percentage < 0 ? 'bg-rose-500' : 'bg-emerald-500'
                            }`}
                            style={{
                        ...
                        <span
                          className={`text-xs ${
                            r.profit_percentage < 0 ? 'text-rose-400' : 'text-slate-300'
                          }`}
                        >
                          {fmtPct(r.profit_percentage)}
                        </span>
      ...
      <div className="md:hidden space-y-3">
        ...
        {normalized.map((r) => {
          const isBest = r.total_profit === maxProfit && maxProfit > 0;
          const isWorst = r.total_profit === minProfit && minProfit < 0;
          return <MobileCardRow key={r.id} r={r} isBest={isBest} isWorst={isWorst} />;
        })}
      </div>
    </>
  );
